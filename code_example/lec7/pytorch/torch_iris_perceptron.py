...
data_train, data_test, target_train, target_test = train_test_split(
    ss_iris_subset, target_subset, test_size=0.15, random_state=11, stratify=target_subset)
...
with torch.no_grad():
    ...
    weight = (0.25*torch.randn(size=(2, 1), dtype=torch.float32)
              ).requires_grad_(True)
...
def cost(model_out, target_input):
    ...
    cost = torch.sum(torch.pow((model_out - target_input), 2)) * \
        (1.0/len(model_out))
