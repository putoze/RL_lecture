...
def mcc_q(simenv, gamma, num_episodes, max_episode_len, printqPeriod=None, prng_seed=789):
    ...
    q=np.zeros((simenv.numStates,simenv.numActions),dtype=np.float32)  #the q-function     
    tot_first_visits=np.zeros((simenv.numStates,simenv.numActions),dtype=np.int32) #total number of first visits per (state, action) across all episodes
    ...
    for episode in range(num_episodes):
        episode_rewards=[] #state,action,reward,first-visit-status sequence of an episode (s,a,r,firstVisit) tuple
        ...
        while episode_length < max_episode_len:            
            ...
            #
            first_visit=False
            if not visited[current_state,action]: first_visit=True
            visited[current_state,action]=True
            
            episode_rewards.append((current_state, action, reward, first_visit))
        ...
        for event in reversed(episode_rewards):
            ...
            if first_visit:
                ...
                q[state,action]=q[state,action]+(1.0/tot_first_visits[state,action])*(tot_return-q[state,action]) #incremental update of q(s,a) averages
