...
#
#  Implementation: Epsilon-greedy expected Sarsa
#
...
def esarsa(simenv, init_state, gamma, epsilon, alpha, num_episodes, max_episode_len, decayEpsilon=True, showPlots=False, prng_seed=789):
    ...
    for episode in range(num_episodes):
        ...
        while episode_length < max_episode_len:
            ...
            q[state,action]=q[state,action]+alpha*(reward+gamma*np.sum(egp.policy(q,next_state)*q[next_state])-q[state,action]) #incremental update of q(s,a)
