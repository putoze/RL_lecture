#
#Expected Sarsa optimal policy estimation demo
...
#################################################################
#
# 1. Estimate optimal policy & q using expected sarsa method
...
(policy,q)=esarsa(cm,cm.currentState(),0.9,1.0,0.1,500,100,decayEpsilon=True,showPlots=show_plots)
...
tot_steps=cm.policy2gif(policy,[0,3],'cm1d_esarsa1')
print('Total steps starting from {}: {}'.format([0,3],tot_steps))
tot_steps=cm.policy2gif(policy,[0,2],'cm1d_esarsa2')
...
#Compute pi(s) & q(s,a) using expected sarsa method
(policy,q)=esarsa(cm,cm.currentState(),0.9,1.0,0.4,1500,250,decayEpsilon=True,showPlots=show_plots)
...
tot_steps=cm.policy2gif(policy,[0,0],'cm2d_esarsa1a')
print('Total steps starting from {}: {}'.format([0,0],tot_steps))
...
#Compute pi(s) & q(s,a) using expected sarsa method
(policy,q)=esarsa(cm,cm.currentState(),0.95,1.0,0.9,750,250,decayEpsilon=True,showPlots=show_plots)
...
#let's walk the policy starting from a few different
# initial states (results are saved to animated .gif files)
tot_steps=cm.policy2gif(policy,[0,0],'cm2d_det_esarsa1')
print('Total steps starting from {}: {}'.format([0,0],tot_steps))
tot_steps=cm.policy2gif(policy,[0,3],'cm2d_det_esarsa2')
print('Total steps starting from {}: {}'.format([0,3],tot_steps))
tot_steps=cm.policy2gif(policy,[3,0],'cm2d_det_esarsa3')
print('Total steps starting from {}: {}'.format([3,0],tot_steps))
...
#Compute pi(s) & q(s,a) using expected sarsa method
(policy,q)=esarsa(cliff,cliff.currentState(),0.9,0.2,0.2,1500,150,decayEpsilon=False,showPlots=show_plots)
...
#let's walk the policy starting from a few different
# initial states (results are saved to animated .gif files)
tot_steps=cliff.policy2gif(policy,[3,0],'cliff_esarsa',maxEpisodeLen=15)
print('Total steps starting from {}: {}'.format([3,0],tot_steps))
