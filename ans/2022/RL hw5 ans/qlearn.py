#
# Q-Learning algorithm, for estimating the optimal policy & q-function
#   q(s,a) [matrix: numStates x numActions], final policy a=pi(s) [column vector: numStates x 1]
#
#  Implementation: Standard q-learning, using epsilon-greedy approach
#
...
#   Q-Learning
#
def qlearn(simenv, init_state, gamma, epsilon, alpha, num_episodes, max_episode_len, decayEpsilon=True, showPlots=False, prng_seed=789):
    '''
    Parameters:
    ...
    for episode in range(num_episodes):
        ...
        while episode_length < max_episode_len:
            ...
            #update q every time step (q-learning update)
            q[state,action]=q[state,action]+alpha*(reward+gamma*np.max(q[next_state])-q[state,action]) #incremental update of q(s,a)
